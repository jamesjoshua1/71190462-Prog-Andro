...
class SearchActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        searchBtn.setOnClickListener {
            listMusik.clear()
            firestore.collection("musik").whereEqualTo("judulLagu", searchTxv.text.toString()).get()
                .addOnSuccessListener { documents ->
                    for (document in documents) {
                        listMusik.add(
                            Musik(
                                "${document.data["judulLagu"]}",
                                "${document.data["namaPenyanyi"]}",
                                "${document.data["judulAlbum"]}",
                                "${document.data["genre"]}",
                                "${document.data["tahunRilis"]}",
                                "${document.data["idLagu"]}"
                            )
                        )
                    }
                    createView()
